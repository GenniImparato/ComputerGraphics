...
vec4 computeAmbientLightColor(vec3 normalVec) {
	return (((dot(normalVec, ambientDir) + 1.0) * ambientHighColor) + ((1.0 - dot(normalVec, ambientDir)) * ambientLowColor)) / 2.0;
}
...
void main() {
	...
	// lambert diffuse without specular
	  outColor = clamp(vec4(lambertDiffuseColor1 *LAOn + lambertDiffuseColor2 * LBOn + lambertDiffuseColor3 * LCOn + lambertDiffuseColor4 * LDOn + ambientComponent.rgb + mEmitColor.rgb, texColor.a),0.0, 1.0);
